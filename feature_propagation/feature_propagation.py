...
class EmbeddingPropagation(torch.nn.Module):
    ...
    def forward(self, x, propagator=None):
        return feature_propagation(x, self.alpha, self.rbf_scale, self.norm_prop, propagator=propagator)
...
def feature_propagation(x, alpha, rbf_scale, norm_prop, propagator=None):
